...
var BugDispatch = {
    ...
    initialize: function (a) {
      ...
      this.options.canDie && this.modes.push('die');
      -1 == this.modes.indexOf(this.options.mouseOver) && (this.options.mouseOver = 'random');
      this.transform = null;
      this.transforms = {
        Moz: function (a) {
      ...
      if ('transform' in document.documentElement.style) this.transform = this.transforms.w3c;
        ...
        for (var b = ['Moz', 'webkit', 'O', 'ms', 'Khtml'], c = 0, c = 0; c < b.length; c++)
      ...
      if (this.transform) {
        ...
        for (
          var b =
              'multiply' === this.options.mouseOver
                ? this.options.minBugs
                : this.random(this.options.minBugs, this.options.maxBugs, !0),
            c = 0,
            d = this,
            c = 0;
          c < b;
          ...
          var e = SpawnBug();
          a.wingsOpen = this.options.canFly ? (0.5 < Math.random() ? !0 : !1) : !0;
          a.walkSpeed = this.random(this.options.minSpeed, this.options.maxSpeed);
          e.initialize(this.transform, a);
          this.bugs.push(e);
        }
        for (c = 0; c < b; c++)
          (a = this.random(this.options.minDelay, this.options.maxDelay, !0)),
            (e = this.bugs[c]),
            setTimeout(
              (function (a) {
                return function () {
            ...
            d.add_events_to_bug(e);
        this.options.monitorMouseMovement &&
    ...
    stop: function () {
      for (var a = 0; a < this.bugs.length; a++) this.bugs[a].stop();
    },
    end: function () {
      for (var a = 0; a < this.bugs.length; a++) this.bugs[a].stop(), this.bugs[a].remove();
    },
    reset: function () {
      this.stop();
    ...
    killAll: function () {
      for (var a = 0; a < this.bugs.length; a++) this.bugs[a].die();
    },
    add_events_to_bug: function (a) {
      var b = this;
      a.bug &&
        (a.bug.addEventListener
          ? a.bug.addEventListener('mouseover', function (c) {
              b.on_bug(a);
            })
          : a.bug.attachEvent &&
            a.bug.attachEvent('onmouseover', function (c) {
              b.on_bug(a);
            }));
    },
    check_if_mouse_close_to_bug: function (a) {
      if ((a = a || window.event)) {
        var b = 0,
          c = 0;
        a.client && a.client.x
          ? ((b = a.client.x), (c = a.client.y))
          : a.clientX
          ? ((b = a.clientX), (c = a.clientY))
          : a.page && a.page.x
          ? ((b = a.page.x - (document.body.scrollLeft + document.documentElement.scrollLeft)),
            (c = a.page.y - (document.body.scrollTop + document.documentElement.scrollTop)))
          : a.pageX &&
            ((b = a.pageX - (document.body.scrollLeft + document.documentElement.scrollLeft)),
            (c = a.pageY - (document.body.scrollTop + document.documentElement.scrollTop)));
        a = this.bugs.length;
        for (var d = 0, d = 0; d < a; d++) {
          var e = this.bugs[d].getPos();
          e &&
            Math.abs(e.top - c) + Math.abs(e.left - b) < this.options.eventDistanceToBug &&
            !this.bugs[d].flyperiodical &&
            this.near_bug(this.bugs[d]);
        }
      }
    },
    near_bug: function (a) {
    ...
    on_bug: function (a) {
      if (a.alive) {
        ...
        'random' === b && (b = this.modes[this.random(0, this.modes.length - 1, !0)]);
...
var Bug = {
    options: { wingsOpen: !1, walkSpeed: 2, flySpeed: 40, edge_resistance: 50, zoom: 10 },
      ...
      if (this.animating && this.alive) {
        ...
        if (
          !(
            40 > c ||
            (200 < c && (c = 200),
            (this._lastTimestamp = a),
            0 >= --this.toggle_stationary_counter && this.toggleStationary(),
            this.stationary)
          )
        ) {
          if (
            0 >= --this.edge_test_counter &&
            this.bug_near_window_edge() &&
            ((this.angle_deg %= 360),
            0 > this.angle_deg && (this.angle_deg += 360),
            15 < Math.abs(this.directions[this.near_edge] - this.angle_deg))
          ) {
          ...
          if (0 >= --this.small_turn_counter)
            ...
            this.large_turn_angle_deg -= a;
            this.angle_deg += a;
          }
          this.angle_rad = this.deg2rad(this.angle_deg);
          a = Math.cos(this.angle_rad) * this.options.walkSpeed * (c / 100);
          c = -Math.sin(this.angle_rad) * this.options.walkSpeed * (c / 100);
          this.moveBug(this.bug.left + a, this.bug.top + c, 90 - this.angle_deg);
          this.walkFrame();
        }
      }
    },
    makeBug: function () {
      if (!this.bug) {
        ...
        b.style.width = this.options.bugWidth + 'px';
    ...
    setPos: function (a, b) {
      this.bug.top =
        a ||
        this.random(this.options.edge_resistance, document.documentElement.clientHeight - this.options.edge_resistance);
      this.bug.left =
        b ||
        this.random(this.options.edge_resistance, document.documentElement.clientWidth - this.options.edge_resistance);
    ...
    drawBug: function (a, b) {
      ...
      a && b ? this.setPos(a, b) : this.setPos(this.bug.top, this.bug.left);
      this.inserted || ((this.inserted = !0), document.body.appendChild(this.bug));
    },
    ...
    walkFrame: function () {
      this.bug.style.backgroundPosition =
        -1 * this.walkIndex * this.options.bugWidth +
        'px ' +
        (this.wingsOpen ? '0' : '-' + this.options.bugHeight + 'px');
      this.walkIndex++;
    ...
    flyIn: function () {
      this.bug || this.makeBug();
      this.stop();
      var a = Math.round(4 * Math.random() - 0.5),
        b = document,
        c = b.documentElement,
      ...
      0 > a && (a = 0);
      0 === a
        ? ((a = -2 * this.options.bugHeight), (b *= Math.random()))
        : 1 === a
        ? ((a = Math.random() * c), (b += 2 * this.options.bugWidth))
        : 2 === a
        ? ((a = c + 2 * this.options.bugHeight), (b *= Math.random()))
        : ((a = Math.random() * c), (b = -3 * this.options.bugWidth));
      this.bug.style.backgroundPosition =
        -3 * this.options.bugWidth + 'px ' + (this.wingsOpen ? '0' : '-' + this.options.bugHeight + 'px');
      this.bug.top = a;
      this.bug.left = b;
      this.drawBug();
      a = {};
    ...
    walkIn: function () {
      ...
      0 > a && (a = 0);
      0 === a
        ? ((a = -1.3 * this.options.bugHeight), (b *= Math.random()))
        : 1 === a
        ? ((a = Math.random() * c), (b += 0.3 * this.options.bugWidth))
        : 2 === a
        ? ((a = c + 0.3 * this.options.bugHeight), (b *= Math.random()))
        : ((a = Math.random() * c), (b = -1.3 * this.options.bugWidth));
      this.bug.style.backgroundPosition =
        -3 * this.options.bugWidth + 'px ' + (this.wingsOpen ? '0' : '-' + this.options.bugHeight + 'px');
      this.bug.top = a;
      this.bug.left = b;
    ...
    flyOff: function () {
      ...
      0 === a
        ? ((b.top = -200), (b.left = Math.random() * c))
        : 1 === a
        ? ((b.top = Math.random() * d), (b.left = c + 200))
        : 2 === a
        ? ((b.top = d + 200), (b.left = Math.random() * c))
        : ((b.top = Math.random() * d), (b.left = -200));
      this.startFlying(b);
    },
    die: function () {
      this.stop();
    ...
    drop: function (a) {
      var b = this.bug.top,
        ...
        e = window.innerHeight || d.clientHeight || c.clientHeight,
        e = e - this.options.bugHeight,
        f = this.random(0, 20, !0);
      Date.now();
      var g = this;
          ...
          (this.bug.style.bottom =
            Math.ceil(
              (this.options.bugWidth * this.zoom - this.options.bugHeight * this.zoom) / 2 -
                (this.options.bugHeight / 2) * (1 - this.zoom)
            ) + 'px'),
          (this.bug.style.left = this.bug.left + 'px'),
          (this.bug.style.backgroundPosition = '-' + 2 * e * this.options.bugWidth + 'px 100%'),
          this.twitch(e))
        : ((this.dropTimer = requestAnimFrame(function (a) {
            g.dropping(a, b, c, d, e);
          })),
          20 > a ||
            ((this.angle_deg = (this.angle_deg + d) % 360),
            (this.angle_rad = this.deg2rad(this.angle_deg)),
            this.moveBug(this.bug.left, f, this.angle_deg)));
    },
    twitch: function (a, b) {
      b || (b = 0);
      var c = this;
    ...
    random: function (a, b, c) {
      if (a == b) return a;
      a = Math.round(a - 0.5 + Math.random() * (b - a + 1));
      return c ? (0.5 < Math.random() ? a : -a) : a;
    },
    ...
    bug_near_window_edge: function () {
      this.near_edge = 0;
      this.bug.top < this.options.edge_resistance
        ? (this.near_edge |= this.NEAR_TOP_EDGE)
        : this.bug.top > document.documentElement.clientHeight - this.options.edge_resistance &&
          (this.near_edge |= this.NEAR_BOTTOM_EDGE);
      this.bug.left < this.options.edge_resistance
        ? (this.near_edge |= this.NEAR_LEFT_EDGE)
        : this.bug.left > document.documentElement.clientWidth - this.options.edge_resistance &&
          (this.near_edge |= this.NEAR_RIGHT_EDGE);
      return this.near_edge;
    },
    getPos: function () {
      return this.inserted && this.bug && this.bug.style
        ? { top: parseInt(this.bug.top, 10), left: parseInt(this.bug.left, 10) }
        : null;
    },
  },
  SpawnBug = function () {
...
function releaseBugs() {
  new BugController({
    imageSprite: 'assets/ladybug.png',
    bugWidth: 30,
    bugHeight: 30,
    minBugs: 5,
    maxBugs: 10,
  ...
  new BugController({
    imageSprite: 'assets/butterfly.png',
    bugWidth: 60,
    bugHeight: 52,
    minBugs: 5,
    maxBugs: 10,
